...
/* Info semester */
.kotak-info { margin:10px 0 15px 0; padding:10px 12px; border-radius:6px; }
.info-berhasil { background:#e7f7e9; color:#096a2e; border:1px solid #bde5c8; }
.info-peringatan { background:#fff3cd; color:#7a5b00; border:1px solid #ffe08a; }

/* Tabel jadwal */
.tabel-jadwal { width:100%; border-collapse:collapse; background:#fff; border-radius:10px; overflow:hidden; box-shadow:0 2px 6px rgba(0,0,0,0.1); table-layout:fixed; }
...
.tabel-jadwal tr:hover { background:#f7fbff; }
...
@media screen and (max-width: 768px) {
  ...
  .tabel-jadwal, thead, tbody, th, td, tr { display:block; }
...
<div class="area-utama">
  ...
  <?php if(!$id_semester_aktif): ?>
    <div class="kotak-info info-peringatan">Belum ada <b>Semester</b> berstatus <b>Aktif</b>. Tidak ada jadwal yang ditampilkan.</div>
  <?php else: ?>
...
if ($id_semester_aktif && $id_dosen) {
  ...
  if ($q && mysqli_num_rows($q) > 0) {
    while($row = mysqli_fetch_assoc($q)){
      ...
      $modul_html = $modul !== ''
        ? "<a href='../../uploads/".e($modul)."' target='_blank' title='Buka Modul'><i class='fa-solid fa-file-arrow-down' style='font-size:30px;color:#0d6efd;'></i></a>"
